  ...
  module Manifest
    class V01Alpha
        ...
        mounts.each do |m|
          if known_targets.include? m.destination
            raise InvalidManifestError.new(
              ...
              "Duplicated mount destination `#{m.destination}' in #{source}"
            )
          end
          ...
          raise InvalidManifestError.new(
            ...
            "Invalid mount source `#{m.source}' in #{source}: Only secrets are mountable."
          )
        end
      end
      ...
      def initialize(data)
        ...
        @checks.each do |c|
          validate_mounts(c.mounts, "plugin `#{c.plugin}'") if c.mounts.present?
